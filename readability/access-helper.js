...
var readability = {
	...
	getArticleTitle: function () {
		...
		articleTitle.className = 'readability-title';
		articleTitle.innerHTML = '文章标题：' + document.title;
	...
	grabArticle: function (preserveUnlikelyCandidates) {
		...
		for(var nodeIndex = 0; (node = document.getElementsByTagName('*')[nodeIndex]); nodeIndex++)
		{
			...
			if (node.tagName === "DIV") {
				if (node.innerHTML.search(readability.regexps.divToPElementsRe) === -1)	{
					...
					try {
						newNode.innerHTML = node.innerHTML.replace(readability.regexps.noid, '');
		...
		**/
		var articleContent        = document.createElement("DIV");
	        ...
	        articleContent.className     = "readability-content";
...
(function() {
	var cssCode = ".readability-title { width:800px;margin:auto;text-align:center;font-size:24px;line-height:30px;font-weight:bold; }"
				+ ".readability-content { width:800px;margin:auto; }"
				+ ".readability-content * { width:auto !important;padding:auto !important;margin:auto !important;line-height:24px !important;font-size:14px !important; }";
	var MetaParser = function() {
		this.keyword = 'new|news|新闻|finance|财经';
		this.init = function() {
			var article, title;
			if(window.location.pathname == '' ||
				window.location.pathname == '/' ||
				window.location.href.search(/index/) >= 0) {
				return;
			}
			if(this.isNewPage()) {
				this.addCss(cssCode);
				
				title = readability.getArticleTitle();
				title.id = 'ach-title';
				title.setAttribute('tabIndex', '0');
				article = readability.grabArticle();
				article.id = 'ach-article';
				article.setAttribute('tabIndex', '0');
				
				this.ariaChild(document.body);
				
				var p = document.createElement('p');
				p.style.textAlign = 'center';
				p.innerHTML = '已启用脚本助手阅读模式，alt+.恢复原页面，alt+h获取更多帮助。';
				...
				if(title.focus) {
					title.focus();
					if(addAchLink) {
addAchLink('', "javascript:document.getElementById('ach-title').focus();void(0);", '', 't', '_self');
addAchLink('', "javascript:document.getElementById('ach-article').focus();void(0);", '', 'p', '_self');
					}
				}
			}
		};
		this.getMeta = function() {
			var metas = document.getElementsByTagName('meta'), items = [];
			for(var i=0, len=metas.length, content; i<len; i++) {
		...
		this.isNewPage = function() {
			var info = this.getMeta(), regexp = new RegExp(this.keyword);
			info.push(document.title);
			for(var i=0, len=info.length; i<len; i++) {
				if(info[i].search(regexp) > -1) {
					return true;
				}
			}
			return false;
		...
		this.ariaChild = function(d) {
			var a = d.childNodes, l, c;
			if(a) {
				l = a.length;
				for (i = 0; i < l; i += 1) {
					c = a[i];
					if(c.nodeType == 1 && c.tagName != 'SCRIPT') {
